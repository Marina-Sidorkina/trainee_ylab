...
import {ICatalogParams, ICatalogState} from "@src/store/catalog/types";
...
class CatalogState extends StateModule {
   ...
   * @return {Promise<void>}
   */
  async initParams(params: ICatalogParams = {} as ICatalogParams) {
    // Параметры из URl. Их нужно валидирвать, приводить типы и брать толкьо нужные
    const urlParams = qs.parse(window.location.search) as {[key: string]: any};
    let validParams = {} as {[key: string]: any};
   ...
   * @return {Promise<void>}
   */
  async resetParams(params: ICatalogParams = {} as ICatalogParams) {
    // Итоговые параметры из начальных, из URL и из переданных явно
